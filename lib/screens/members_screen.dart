...
class _MembersScreenState extends State<MembersScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      for (var m in mems) {
        ...
        try {
          ...
          for (var h in history) {
            if (h['type'] != null && h['type'].toString().contains('Deficit Penalty Paid')) {
              totalPaidDeficitInterest += ((h['amount'] ?? 0.0) as num).toDouble();
            }
          }
          ...
          while (today.isAfter(cycleDate) || cycleDate.isAtSameMomentAs(today)) {
            ...
            for (var h in history) {
              if (h['type'] != null && !h['type'].toString().contains('Penalty')) {
                 try {
                   ...
                   if (hDate.isBefore(cyclePenaltyDate) || hDate.isAtSameMomentAs(cyclePenaltyDate)) {
                       contributionAsOfCycle += ((h['amount'] ?? 0.0) as num).toDouble();
                   }
                 } catch (_) {}
              }
            }
            ...
            if (currentDeficit > 0) {
               if (today.isAfter(cyclePenaltyDate) || today.isAtSameMomentAs(cyclePenaltyDate)) {
                   generatedDeficitInterest += (currentDeficit * 0.15); // Late 15%
               } else {
                   generatedDeficitInterest += (currentDeficit * 0.10); // Active Grace 10%
               }
            }
            cycleDate = _addOneMonth(cycleDate);
          }
          
          double newDeficitInterest = generatedDeficitInterest - totalPaidDeficitInterest;
          if (newDeficitInterest < 0) newDeficitInterest = 0.0;
          
          double oldDeficitInterest = ((m['deficitInterest'] ?? 0.0) as num).toDouble();
          
          if ((newDeficitInterest - oldDeficitInterest).abs() > 0.01) {
            double lateJoin = ((m['lateJoinInterest'] ?? 0.0) as num).toDouble();
            m['deficitInterest'] = double.parse(newDeficitInterest.toStringAsFixed(2));
            m['totalInterest'] = double.parse((newDeficitInterest + lateJoin).toStringAsFixed(2));
      ...
      if (mounted) setState(() { _allMembers = mems; _isLoading = false; });
    } catch (e) {
      if (mounted) setState(() { _allMembers = []; _isLoading = false; });
    }
  }
  ...
  List<Map<String, dynamic>> get _filteredMembers {
    return _allMembers.where((m) {
      ...
      final matchesStatus = _filterStatus == 'All' || displayStatus == _filterStatus;
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const Center(child: CircularProgressIndicator(color: DashboardTheme.accentColor));
    }
    return Scaffold(
          ...
          children: [
            ...
            Expanded(
              child: MembersList(
                ...
                onEdit: (member) {
                  AddMemberModal.show(context, initialMember: member, onSave: (updated) async {
                    final messenger = ScaffoldMessenger.of(context);
                    setState(() {
                      final i = _allMembers.indexWhere((m) => m['id'] == updated['id']);
                      if (i != -1) _allMembers[i] = updated;
                    });
                    await StorageService.saveMembers(_allMembers);
                    messenger.showSnackBar(SnackBar(content: Text('${updated['name']} updated successfully!')));
                  });
                },
                ...
                onDelete: (id) {
                  showDialog(context: context, builder: (ctx) => AlertDialog(
                    ...
                    content: const Text('Are you sure you want to completely delete this member? This action cannot be undone.'),
                    actions: [
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          Navigator.pop(ctx);
                          final messenger = ScaffoldMessenger.of(context);
                          setState(() { _allMembers.removeWhere((m) => m['id'] == id); });
                          await StorageService.saveMembers(_allMembers);
                          messenger.showSnackBar(const SnackBar(content: Text('Member permanently deleted.')));
                        },
        ...
        onPressed: () => AddMemberModal.show(
          ...
          onSave: (newMember) async {
            ...
            messenger.showSnackBar(
              SnackBar(content: Text('${newMember['name']} added successfully! Saved permanently.')),
            );
          },
        ),
